...
1. **巴法云空调实体**：云端只发布“客厅空调”和“卧室空调”两个 Climate 组件；洗衣机、旧“格力空调”和“全屋关空调”主题均已删除。两台空调在 HA 设备注册表中分别归属客厅、卧室。
2. 用户剩余步骤：米家 App → 我的 → 其他平台设备 → 添加 → 巴法 → 输入巴法云私钥（32 位 UID），等设备自动同步。
3. 测试："小爱同学，打开客厅空调 / 客厅空调调到 26 度 / 关闭卧室空调"。HA 内部的“全屋关空调”脚本仍可用于仪表盘与自动化，但不会发布到巴法云。
4. 注意：小米自家设备（电视、音箱等）无需走巴法云，原生支持小爱；巴法云只支持部分实体类型（开关、灯、空调等），不支持按键类型。
5. 私钥存放在 HA 配置（`.storage/core.config_entries`），如需重置请到巴法云后台更换。
6. **巴法云稳定性修复（2026-08-18）**：免费 API（api.bemfa.com）经常 504/超时，导致集成启动“设置失败”。已给 `custom_components/bemfa/http.py` 增加 5 次重试（10s 超时+退避），并改造 `service.py`：先连 MQTT，主题拉取放到后台（12 轮×5 次重试），失败不再阻塞启动。重启后如果实体/同步没恢复，等 1～3 分钟自动重试即可；云端 `hassping` 主题时间持续更新即表示桥在线。

## 10. 自动化场景

在 设置 → 自动化与场景 中创建：

- 一键全关空调：`script.shutdown_all_ac` 已覆盖客厅与卧室两台格力空调的 `climate.turn_off`。
...
- 小爱原生回答失败时，`ai_voice_bridge` 才调用 `conversation.deepseek`。失败识别覆盖“不知道、不清楚、没有答案、暂不支持、没听懂、正在学习、抱歉”等常见否定答复；固定的离家、回家、晚安、报温度、洗衣机状态和所有控制类指令均被排除，继续由原自动化处理。
